    ...
    fn log(&self) {
        ...
        terminal
            .draw(|frame| {
                ...
                let rows = rows.iter().map(|&(&coder_id, (time, state))| {
                    Row::new([
                        time.as_millis().to_string().blue(),
                        CODER_NAMES[coder_id as usize % CODER_NAMES.len()]
                            .to_string()
                            .fg(Color::Indexed((coder_id % 256) as u8)),
                        state.styled().bold(),
                    ])
                });
                let table =
                    Table::new(rows, constraints![==20%, ==20%, >=20]).block(Block::bordered());
...
impl CoderState {
    fn styled(&self) -> Span<'static> {
        ...
        match self {
            CoderState::Compiling => repr.green(),
            CoderState::Debugging => repr.yellow(),
            CoderState::Refactoring => repr.magenta(),
            CoderState::AcquiredDongle(_) => repr.dim(),
            CoderState::ReleasedDongle(_) => repr.dim(),
            CoderState::BurnedOut => repr.red(),
        }
    }
}
